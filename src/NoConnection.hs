...
newtype NoConnection db (m :: * -> *) = NoConnection db

data NoConnectionStmt db = NoConnectionStmt db (Set Var) Query (Set Var)

instance DBConnection0 (NoConnection db m) where
    dbClose _ = return ()
    dbBegin _ = return ()
