...
generateICATDef (Stmt tablename coldefs) = do
    ...
    if prefix `elem` []
        then
            ...
            let q1 = [| Pred $(stringQ predname) (PredType ObjectPred $keysq) |]
            let propPred (ColDef key2 keytype2 _) = [| Pred $(stringQ (colNameToPredName prefix key2)) (PredType PropertyPred ($keysq ++  [Property $(stringQ (show keytype2))])) |]
